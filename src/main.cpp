...
[[noreturn]] void pollEvent() {
    while (true) {
        ...
        zmq::recv_result_t result =
                zmq::recv_multipart(*g->zmg_pull, std::back_inserter(recv_msgs));

        // loop through recv_msgs
        for (auto & msg: recv_msgs) {
            auto *gdmp_packet = new gdmp::Packet();
            gdmp_packet->ParseFromArray(msg.data(), msg.size());
...
global->zmg_ctx = new zmq::context_t();
global->zmg_push = new zmq::socket_t(*global->zmg_ctx, zmq::socket_type::pub);
global->zmg_pull = new zmq::socket_t(*global->zmg_ctx, zmq::socket_type::sub);
