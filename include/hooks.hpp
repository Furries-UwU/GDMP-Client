...
class $modify(cocos2d::CCScheduler){void update(float dt){CCScheduler::update(dt);
...
auto button = CCMenuItemSpriteExtra::create(
    buttonSprite, this, menu_selector(MultiplayerLayer::switchToCustomLayerButton)
);
...
class $modify(PlayLayer){bool init(GJGameLevel * level){if (!PlayLayer::init(level)) return false;
...
if (global->connected) {
    ...
    // PlayerObject::getSecondaryColor has an binding on mac but not anywhere else and i cba to find
    // it lmao Binding doesn't exist, caues an error
    ...
    player_join->set_username(gameManager->m_playerName);
...
void update(float p0) {
    ...
    if (global->connected) {
        ...
        auto enetPacket = enet_packet_create(
            buffer, size, ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT
        ); /* is this how u do unreliable packets? */
        free(buffer);
...
){void pushButton(int i, bool pl){if (pl) Global::get()->P1_pushing = true;
